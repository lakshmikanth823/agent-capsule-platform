...
import {
  ...
  AccessManager,
} from "../services/edge-proxy/dist/index.js";
import {
  CapsuleLifecycleManager,
  DevMockSandboxDriver,
} from "../packages/sandbox-driver/dist/index.js";
...
async function main() {
  ...
  const emptyProjectDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "capsule-phase0-app-"),
  );
  const configDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "capsule-cli-config-"),
  );
  ...
  const lifecycleManager = new CapsuleLifecycleManager({
  ...
  try {
    ...
    await publishCommand({
      description: "Phase 0 reference release",
      json: true,
    });
    ...
    console.log(
      `  + Publish finished. App live at: http://phase0-app.apps.localhost:8080`,
    );
    console.log(
      `  + Time from publish to live URL: \x1b[32m${publishToLiveSec}s\x1b[0m`,
    );
    ...
    console.log(
      `  ✓ Publish-to-URL threshold check PASSED (${publishToLiveSec}s < ${MAX_PUBLISH_SECONDS}s)`,
    );
    ...
    console.log(
      `  + Shared 'phase0-app' with bob@example.com (role: employee)`,
    );
    results.steps.share = {
      success: true,
      user: "bob@example.com",
    ...
    await page.goto("http://phase0-app.apps.localhost:8080/", {
    ...
    await Promise.all([
      ...
      page.evaluate(() => {
        ...
        const bobLink = links.find(
          (el) => el.textContent && el.textContent.includes("Bob"),
        );
    ...
    console.log(
      `  + Time from opening link to first render: \x1b[32m${linkToRenderMs}ms\x1b[0m`,
    );
    ...
    console.log(
      `  + Host-only cookie verified: ${appCookie?.name}=${appCookie?.value?.substring(0, 15)}... (domain: ${appCookie?.domain})`,
    );
    ...
    console.log(
      `  + Sending wake request to http://phase0-app.apps.localhost:8080/api/identity...`,
    );
    ...
    console.log(
      `  + Wake response received (${identityRes.status} OK):`,
      JSON.stringify(identityRes.data),
    );
    console.log(
      `  + Cold-start time after suspend: \x1b[32m${coldStartMs}ms\x1b[0m`,
    );
    ...
    console.log(
      `  1. Publish to Live URL:        ${publishToLiveSec}s (Threshold: < 60s) -> PASSED`,
    );
    console.log(
      `  2. Link to First Render:       ${linkToRenderMs}ms (Browser OAuth handshake)`,
    );
    console.log(
      `  3. Cold-Start after Suspend:   ${coldStartMs}ms (Wake-on-request)`,
    );
    console.log(
      "================================================================\n",
    );
