...
const ARTIFACT_DIR = 'C:/Users/dell/.gemini/antigravity/brain/5e7a2a9c-243c-47e4-844c-fba9a2306259';
...
async function main() {
  ...
  try {
    ...
    // -------------------------------------------------------------
    console.log('[*] Navigating to http://platform.localhost:8080/ for Screen 1 (Sign-in)...');
    await page.goto('http://platform.localhost:8080/', { waitUntil: 'networkidle0' });
    ...
    await page.waitForFunction(() => document.body.innerText.includes('Leave Tracker'), { timeout: 8000 });
    await wait(800);

    await takeScreenshot('screenshot_02_dashboard_apps.png', 'Screen 2: Dashboard Apps List');
    ...
    await page.waitForFunction(() => document.body.innerText.includes('Overview'), { timeout: 5000 });
    await wait(500);
    ...
    await takeScreenshot('screenshot_03_app_detail.png', 'Screen 3: App Detail & Logs');
    ...
    await page.waitForFunction(() => document.body.innerText.includes('Permission Preview'), { timeout: 5000 });
    await wait(1000);

    await takeScreenshot('screenshot_04_share_permission_preview.png', 'Screen 4: Share & Permission Preview');
    ...
    await page.evaluate(() => {
      const buttons = Array.from(document.querySelectorAll('button'));
      const historyTab = buttons.find((b) => b.textContent?.includes('Version History'));
      if (historyTab) historyTab.click();
    });
    await wait(800);

    await takeScreenshot('screenshot_05_version_history.png', 'Screen 5: Version History');
