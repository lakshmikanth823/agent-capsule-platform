...
const response = await sdk.ai.chat({
  messages: [
    { role: 'user', content: 'Summarize the employee handbook.' }
  ],
  temperature: 0.2
});
console.log(response.content);
console.log(`Tokens: ${response.usage.total_tokens}, Cost: $${response.usage.estimated_cost_usd}`);
...
console.log(`Monthly spend: $${usage.current_spend_usd} / $${usage.monthly_budget_usd}`);
```
