-- Software Capsule Control-Plane Database Schema
-- Target: PostgreSQL 16+
-- Scope: users, organizations, apps/capsules, versions, shares, audit
-- App runtime data is NOT stored here; Capsule application data uses per-Capsule SQLite.
-- IDs use UUIDs. Timestamps are UTC timestamptz.
...
CREATE TABLE users (
    ...
    -- A subject is unique within an identity issuer.
...
CREATE INDEX idx_org_members_org_role
    ON organization_members(organization_id, platform_role);

-- ============================================================
-- 4. APPS / SOFTWARE CAPSULES
-- ============================================================
...
CREATE TABLE apps (
    ...
    -- Stable manifest/application identifier.
    ...
    -- Last accepted manifest, retained as control-plane metadata.
...
CREATE INDEX idx_apps_org_status
    ON apps(organization_id, status);

CREATE INDEX idx_apps_owner
    ON apps(owner_user_id);

CREATE INDEX idx_apps_updated
    ON apps(updated_at DESC);

-- ============================================================
-- 5. APP MEMBERSHIPS / SHARES
-- ============================================================
...
CREATE TABLE app_shares (
    ...
    -- Exactly one of user_id or organization-wide share applies.
    user_id             uuid REFERENCES users(id) ON DELETE CASCADE,

    -- Application role, e.g. employee / manager / hr.
    app_role            varchar(64),

    -- Whether the share grants access.
    status              varchar(20) NOT NULL DEFAULT 'active'
    ...
    -- Optional metadata for future group-based sharing.
    metadata            jsonb NOT NULL DEFAULT '{}'::jsonb,

    CONSTRAINT chk_app_share_expiry
        CHECK (expires_at IS NULL OR expires_at > granted_at),

    CONSTRAINT uq_app_user_share UNIQUE (app_id, user_id, app_role)
);

CREATE INDEX idx_app_shares_app_status
    ON app_shares(app_id, status);

CREATE INDEX idx_app_shares_user
    ON app_shares(user_id);

CREATE INDEX idx_app_shares_expiry
    ON app_shares(expires_at)
    WHERE expires_at IS NOT NULL;

-- Organization-wide default sharing is represented separately so
-- it cannot be confused with a user-specific grant.
...
CREATE TABLE app_versions (
    ...
    -- Snapshot captured with every deployment.
    ...
    CONSTRAINT chk_published_has_timestamp
        CHECK (
...
CREATE INDEX idx_app_versions_app
    ON app_versions(app_id, version_number DESC);

CREATE INDEX idx_app_versions_status
    ON app_versions(status);
...
CREATE TABLE audit_events (
    ...
    -- Human actor when available.
    actor_user_id       uuid REFERENCES users(id) ON DELETE SET NULL,

    -- Agent/tool that initiated the operation, if applicable.
...
CREATE INDEX idx_audit_app_time
    ON audit_events(app_id, occurred_at DESC);
...
-- ============================================================
-- 8. OPTIONAL CAPABILITY APPROVAL RECORD
-- ============================================================
-- Required by the PRD/TRD escalation model. Keeping approvals
-- separate from the manifest makes human approval auditable.
...
CREATE TABLE capability_approvals (
    ...
    CONSTRAINT chk_capability_decision_actor
        CHECK (
...
CREATE INDEX idx_capability_approvals_app
    ON capability_approvals(app_id, status);
