...
describe("Edge Proxy - Sharing, Roles, and Immediate Revocation", () => {
  ...
  it("should enforce each role's allowed and denied access", async () => {
    ...
    const aliceTicket = new URL(
      aliceTicketRes.headers.location || "",
    ...
    const aliceCookie =
      aliceCallback.headers["set-cookie"]?.[0]?.split(";")[0] || "";
    ...
    const charlieTicket = new URL(
      charlieTicketRes.headers.location || "",
    ).searchParams.get("ticket");
  ...
  it("should handle group membership changes for group-based sharing", async () => {
    ...
    const bobCookie =
      bobCallback.headers["set-cookie"]?.[0]?.split(";")[0] || "";
  ...
  it("Acceptance: owner shares app with colleague, colleague opens it, unsharing blocks them immediately", async () => {
    ...
    expect(blockedRes.body).toContain(
      "Access to this capsule has been revoked",
    );
