...
describe("Edge Proxy Service", () => {
  ...
  it("should extract capsule id from subdomain on apps.localhost", () => {
    expect(
      extractSubdomainCapsuleId(
        "leave-tracker.apps.localhost:8080",
        "apps.localhost",
      ),
    ).toBe("leave-tracker");
    expect(
      extractSubdomainCapsuleId(
        "team-dashboard.apps.localhost",
        "apps.localhost",
      ),
    ).toBe("team-dashboard");
    expect(
      extractSubdomainCapsuleId("platform.localhost:8080", "apps.localhost"),
    ).toBeNull();
    expect(
      extractSubdomainCapsuleId("apps.localhost:8080", "apps.localhost"),
    ).toBeNull();
  });
...
describe.skipIf(!hasDocker)(
  "Edge Proxy Acceptance Test (with DockerDevDriver & Leave Tracker App)",
  () => {
    let server: http.Server;
    let serverPort: number;
    ...
    it("Acceptance: opening the sample app URL in browser logs in with mock IdP and shows identity", async () => {
      ...
      expect(initialRes.headers.location).toContain(
