...
export function createProgram(): Command {
  ...
  program
    .name("capsule")
    .description(
      "Software Capsule Platform CLI for publishing, sharing, and managing small apps",
    )
    .version("0.1.0");
  ...
  program
    ...
    .option(
      "--manifest <path>",
      "Path to manifest file (defaults to capsule.manifest.yaml)",
    )
    .option("--json", "Output machine-readable JSON")
  ...
  program
    ...
    .option(
      "--expected-version <n>",
      "Enforce optimistic concurrency against current version",
    )
    .option(
      "--dry-run",
      "Run validation and admission checks without deploying",
    )
    .option("--wait", "Wait for deployment to become active")
    .option("--manifest <path>", "Path to manifest file")
    .option(
      "--idempotency-key <key>",
      "Custom idempotency key (defaults to UUIDv4)",
    )
    .option("--json", "Output machine-readable JSON")
  ...
  shareCmd
    .command("add")
    .description(
      "Share a capsule with a user or group and assign an application role",
    )
    .requiredOption(
      "--role <role>",
      "Application role to assign (e.g. employee, manager)",
    )
  ...
  program
    ...
    .description(
      "Revoke an existing sharing assignment (alias for `capsule share revoke`)",
    )
    .option("--app <appKey>", "Target capsule ID or key")
    .option("--json", "Output machine-readable JSON")
  ...
  program
    ...
    .requiredOption("--version <n>", "Target version number", (v) =>
      parseInt(v, 10),
    )
    .option(
      "--mode <mode>",
      "Rollback mode: code-only or code-and-data (default: code-only)",
      "code-only",
    )
  ...
  const auditCmd = program
    .command("audit")
    .description(
      "Inspect, verify, export, and manage tamper-evident audit logs",
    );
  ...
  auditCmd
    ...
    .option(
      "--outcome <outcome>",
      "Filter by outcome (success, denied, failed)",
    )
  ...
  auditCmd
    .command("verify")
    .description(
      "Cryptographically verify hash chain continuity and tamper evidence",
    )
    .option("--json", "Output machine-readable JSON")
  ...
  program
    .command("inventory")
    .description(
      "List and inspect application inventory across your organization (FR-036)",
    )
    .option("--org <orgId>", "Filter by organization ID")
    .option(
      "--status <status>",
      "Filter by application status (active, suspended, archived)",
    )
  ...
  program
    ...
    .description(
      "Configure governance lifecycle parameters for a capsule (FR-034/35)",
    )
    ...
    .option(
      "--purge-days <days>",
      "Retention window in days between archival and purge",
    )
    .option("--json", "Output machine-readable JSON")
