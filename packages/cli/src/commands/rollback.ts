...
export async function rollbackCommand(options: RollbackOptions): Promise<void> {
  ...
  if (options.version === undefined || isNaN(options.version)) {
    outputError(
      new CliError({
        ...
        message: 'Must specify target version to roll back to with --version <n>.',
        exitCode: 1,
  ...
  try {
    ...
    outputResult(
      ...
      options,
      () => {
        console.log(`\x1b[32m✓ Rollback to version ${result.target_version_number} succeeded!\x1b[0m`);
        console.log(`  Active Version:    v${result.version_number} (${result.mode})`);
        console.log(`  Data Restored:     ${result.data_restored ? 'Yes (from snapshot)' : 'No (code-only)'}`);
    ...
    if (err instanceof CliError && err.code === 'CONFIRMATION_REQUIRED') {
      ...
      console.log(`\x1b[33m\x1b[1mWARNING: This operation restores application data from the snapshot associated with version ${options.version}.\x1b[0m`);
      if (warning) {
        console.log(`Current data created after that snapshot may be lost (${warning.current_records} current records, ${warning.estimated_records_lost} estimated lost).`);
