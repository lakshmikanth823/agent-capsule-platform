...
function resolveAppKey(explicitAppKey?: string): string {
  ...
  throw new CliError({
    ...
    message: 'Could not determine target capsule. Specify <app> or run from a project directory.',
    exitCode: 1,
...
export async function suspendCommand(appArg?: string, options: SuspendOptions = {}): Promise<void> {
  let appKey: string;
  try {
  ...
  if (!reason) {
    outputError(
      new CliError({
        ...
        message: 'A non-empty suspension reason is required (use --reason "<reason>").',
        exitCode: 2,
