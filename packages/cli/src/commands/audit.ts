...
export async function auditListCommand(
  ...
  try {
    ...
    const res = await client.request(
      `/v1/organizations/${orgId}/audit/events${query}`,
    );
    ...
    outputResult(res, options, () => {
      ...
      console.log(
        "SEQ #  | TIMESTAMP                 | ACTION               | OUTCOME | ACTOR / TOOL            | EVENT HASH",
      );
      console.log(
        "--------------------------------------------------------------------------------------------------",
      );
      for (const e of items) {
        ...
        const actor = `${e.actor_agent || "user"}:${e.actor_tool || "web"}`
          .slice(0, 23)
          .padEnd(23);
...
export async function auditVerifyCommand(
  ...
  try {
    ...
    const report = await client.request(
      `/v1/organizations/${orgId}/audit/verify`,
      {
        method: "POST",
      },
    );
    ...
    outputResult(report, options, () => {
      if (report.valid) {
        ...
        console.log(
          `Sequence Range: #${report.first_sequence ?? 0} to #${report.last_sequence ?? 0}`,
        );
        console.log(
          report.checkpoint
        ...
        console.log(
          "Result: Zero sequence gaps, zero reorderings, and zero data alterations detected.",
        );
      } else {
        ...
        console.log(
          "WARNING: Database mutation or row deletion detected in violation of immutability invariant!",
        );
      }
    });
  } catch (err: any) {
...
export async function auditRetentionCommand(
  ...
  try {
    const orgId = await resolveOrgId(client);
    const res = await client.request(
      `/v1/organizations/${orgId}/audit/retention/enforce`,
      {
        method: "POST",
      },
    );
