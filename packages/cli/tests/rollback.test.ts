...
describe('CLI Rollback & Versions Commands', () => {
  ...
  it('should reject invalid rollback mode', async () => {
    ...
    await rollbackCommand({ app: 'test-app', version: 1, mode: 'invalid-mode', json: true });
  ...
  it('should execute code-only rollback successfully', async () => {
    mockClient.rollback.mockResolvedValueOnce({
      ...
      mode: 'code_only',
      recovery_snapshot_ref: 'capsules/app-123/snapshots/pre_rollback_v3.sqlite',
      data_restored: false,
    ...
    await rollbackCommand({ app: 'test-app', version: 1, mode: 'code-only', json: true });
  ...
  it('should execute code-and-data rollback with confirmation', async () => {
    mockClient.rollback.mockResolvedValueOnce({
      ...
      mode: 'code_and_data',
      recovery_snapshot_ref: 'capsules/app-123/snapshots/pre_rollback_v4.sqlite',
      data_restored: true,
      created_at: new Date().toISOString(),
    });
