...
describe("Capsule CLI Package", () => {
  ...
  it("should scaffold complete starter project with capsule init", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(process.cwd(), "test-app", "capsule.manifest.yaml"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(process.cwd(), "test-app", "package.json")),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(process.cwd(), "test-app", "tsconfig.json")),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(process.cwd(), "test-app", "src", "index.ts")),
    ).toBe(true);

    // Clean up created test-app in cwd
    try {
      fs.rmSync(path.join(process.cwd(), "test-app"), {
        recursive: true,
        force: true,
      });
    } catch {
...
`,
    ...
    await expect(
      validateCommand({ manifest: invalidManifest, json: true }),
