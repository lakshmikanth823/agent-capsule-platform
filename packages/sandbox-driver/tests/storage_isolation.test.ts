...
describe("Prompt 09: Storage Isolation, Persistence, and Identity Verification", () => {
  ...
  beforeEach(() => {
    tmpBaseDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "capsule-storage-test-"),
    );
  ...
  it("data persists across suspend and resume", async () => {
    ...
    const row = dbAfterResume.get<{ name: string }>(
  ...
  it("one capsule cannot see another capsules database (storage isolation)", async () => {
    ...
    dbA.execute("INSERT INTO secrets VALUES (?, ?)", [
      "secret_a",
      "Capsule-A-Secret-Data",
    ]);
    dbA.close();
    ...
    dbB.execute("INSERT INTO secrets VALUES (?, ?)", [
      "secret_b",
      "Capsule-B-Secret-Data",
    ]);
    dbB.close();

    // Verify isolation: DB A does not have secret_b
    const verifyA = new SQLiteDatabase({ path: dbPathA });
    const rowInA = verifyA.get("SELECT val FROM secrets WHERE key = ?", [
      "secret_b",
    ]);
    ...
    const rowInB = verifyB.get("SELECT val FROM secrets WHERE key = ?", [
      "secret_a",
    ]);
  ...
  it("Acceptance: the sample app stores and reads records per user, and survives a restart", async () => {
    ...
    const bobAfterRestart = dbAfterRestart.query(
      "SELECT * FROM leave_requests WHERE user_id = ?",
      ["bob-user-id"],
    );
