    ...
    global.fetch = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        detail: {
          ...
          current_spend_usd: 25.1,
        },
      }),
    });

    await expect(
      sdk.ai.chat([{ role: "user", content: "Generate text" }], {
        gatewayUrl: "http://localhost:8000",
      }),
    ).rejects.toThrow(AIGatewayError);

    try {
      await sdk.ai.chat([{ role: "user", content: "Generate text" }], {
        gatewayUrl: "http://localhost:8000",
      });
    } catch (err: any) {
      ...
      expect(err.details.current_spend_usd).toBe(25.1);
    }
  });
});
