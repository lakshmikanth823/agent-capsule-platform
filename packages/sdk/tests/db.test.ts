...
describe("@capsule/sdk - Database (SQLite)", () => {
  ...
  it("should execute DDL, INSERT, and queries correctly", () => {
    ...
    const result = db.execute("INSERT INTO users (name, email) VALUES (?, ?)", [
      "Alice",
      "alice@example.com",
    ]);
  ...
  it("should execute transactions with immediate lock (single-writer guarantee)", () => {
    ...
    db.exec(
      "CREATE TABLE counter (val INTEGER); INSERT INTO counter VALUES (0);",
    );
    ...
    const counterAfterRollback = db.get<{ val: number }>(
      "SELECT val FROM counter",
    );
  ...
  it("should export clean database using VACUUM INTO", () => {
    ...
    const note = exportedDb.get<{ text: string }>(
