...
describe("@capsule/sdk - Files (Blob Storage)", () => {
  ...
  it("should block path traversal attempts", async () => {
    await expect(files.put("../outside.txt", "bad")).rejects.toThrowError(
      FileStorageError,
    );
    await expect(files.put("../../etc/passwd", "bad")).rejects.toThrowError(
      FileStorageError,
    );
    await expect(files.get("../outside.txt")).rejects.toThrowError(
      FileStorageError,
    );
    await expect(files.delete("../outside.txt")).rejects.toThrowError(
      FileStorageError,
    );
  });
});
