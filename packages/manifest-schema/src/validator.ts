...
import type { CapsuleManifest, ValidationCheck, ValidationResult, ConnectorDeclaration } from './types.js';
...
export function validateManifest(input: unknown): ValidationResult {
  ...
  if (!isSchemaValid && validateSchema.errors) {
    for (const err of validateSchema.errors) {
      let path = err.instancePath ? err.instancePath.replace(/^\//, '').replace(/\//g, '.') : null;
  ...
  if (manifest.limits && typeof manifest.limits === 'object') {
    ...
    if (limits.memory_mb !== undefined && (limits.memory_mb < 64 || limits.memory_mb > 16384)) {
      checks.push({
        name: 'limits_check',
        status: 'fail',
        code: 'quota_exceeded',
    ...
    if (limits.request_timeout_s !== undefined && (limits.request_timeout_s < 1 || limits.request_timeout_s > 300)) {
      checks.push({
        name: 'limits_check',
        status: 'fail',
        code: 'quota_exceeded',
    ...
    if (limits.max_active_instances !== undefined && limits.max_active_instances !== 1) {
      checks.push({
        name: 'limits_check',
        status: 'fail',
        code: 'quota_exceeded',
  ...
  if (manifest.schedule && Array.isArray(manifest.schedule) && manifest.schedule.length > 0) {
    checks.push({
      name: 'schedule_check',
      status: 'warn',
      code: 'unsupported_capability',
      path: 'schedule',
      message: 'Scheduled execution is deferred until Phase 3 and is not active in Alpha.',
  ...
  if (valid) {
    const normalizedConnectors: ConnectorDeclaration[] | undefined = manifest.capabilities?.connectors
