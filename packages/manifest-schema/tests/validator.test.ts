...
`;
    ...
    expect(result.valid).toBe(true);
    expect(result.errors).toHaveLength(0);
    expect(result.effective_manifest?.roles).toEqual([
      "employee",
      "manager",
      "hr",
    ]);
    ...
    expect(
      result.effective_manifest?.capabilities?.connectors?.[0]?.acts_as,
    ).toBe("viewer");
  });
...
`;
  ...
  it("5. should reject manifest missing required id", () => {
    ...
    const result = validateManifest(manifest);
    expect(result.valid).toBe(false);
    const idErr = result.errors.find(
      (e) => e.path === "id" || e.message.includes("'id'"),
    );
  ...
  it("6. should reject manifest missing required name", () => {
    ...
    const nameErr = result.errors.find(
      (e) => e.path === "name" || e.message.includes("'name'"),
    );
  ...
  it("16. should reject loopback and cloud metadata endpoints", () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.errors.some((e) => e.message.includes("Loopback or metadata")),
    ).toBe(true);
  });
  ...
  it("19. should flag privileged service connector identity as approval_required", () => {
    ...
    expect(result.required_approvals).toContain(
      "connectors.slack.post:service",
    );
  ...
  it("21. should reject request_timeout_s outside 1 - 300 seconds", () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.errors.some((e) => e.path === "limits.request_timeout_s"),
    ).toBe(true);
  });
  ...
  it("22. should reject max_active_instances not equal to 1", () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.errors.some((e) => e.path?.includes("max_active_instances")),
    ).toBe(true);
  });
  ...
  it("24. should issue a warning when schedule is declared in Alpha/MVP", () => {
    ...
    const warn = result.warnings.find(
      (w) => w.code === "unsupported_capability",
    );
    expect(warn).toBeDefined();
