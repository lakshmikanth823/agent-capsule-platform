 ...
 */
export function createCapsuleMcpServer(
  ...
  function requireAuth(client: ApiClient): void {
    if (!client.token) {
      throw new CliError({
        ...
        message:
          "No session credentials found. Authenticate using `capsule login` or set the CAPSULE_TOKEN environment variable.",
        exitCode: 5,
  ...
  Validation result containing 'valid' (boolean), errors, warnings, required approvals, and effective manifest.`,
      inputSchema: {
        manifest_content: z
          ...
          .optional()
          .describe(
            "Raw YAML or JSON string of the manifest to validate offline.",
          ),
        path: z
          .string()
          .optional()
          .describe(
            "Path to capsule.manifest.yaml file (defaults to ./capsule.manifest.yaml).",
          ),
      },
      annotations: {
        readOnlyHint: true,
    ...
    async (args) => {
      try {
        ...
        if (args.manifest_content) {
          ...
          const manifestPath = args.path
            ? path.resolve(cwd, args.path)
            : path.resolve(cwd, "capsule.manifest.yaml");
...
Args:
  ...
  - confirm (boolean, optional): Explicit confirmation required if requesting service identity.`,
      inputSchema: {
        app_id: z
          .string()
          .describe("Target Capsule application ID (e.g. leave-tracker)."),
        manifest_content: z
          .string()
          .optional()
          .describe(
            "Manifest YAML content string. If omitted, reads from local file.",
          ),
        path: z
          .string()
          .optional()
          .describe("Filesystem path to manifest file or project root."),
        description: z
          .string()
          .optional()
          .describe("Change description for this published version."),
        expected_version: z
          .number()
          .optional()
          .describe(
            "Expected current version number for optimistic concurrency.",
          ),
        confirm: z
          .boolean()
          .optional()
          .describe(
            "Must be true if publishing with service identity capabilities.",
          ),
      },
      annotations: {
        readOnlyHint: false,
      ...
      try {
        ...
        if (!valResult.valid) {
          throw new CliError({
            ...
            message:
              valResult.errors[0]?.message || "Manifest validation failed",
        ...
        const requestsServiceIdentity =
          ...
          connectors.some(
            (c: any) => typeof c === "object" && c.acts_as === "service",
          );
        ...
        if (requestsServiceIdentity) {
          assertConfirmation({
            ...
            details: {
              app_id: appKey,
              sensitive_capability: "service_identity",
            },
          });
        }
        ...
        const publishResponse = await client.publish(
          appKey,
          {
            ...
            change_description:
              args.description || "Published via Capsule MCP adapter",
        ...
        const versionNum =
          publishResponse?.version?.version_number ||
          publishResponse?.version_number ||
          1;
        ...
        const outputData = {
          ...
          details: formatUntrustedData(
            publishResponse,
            "control_plane_publish_response",
          ),
        };
...
Args:
  ...
  - confirm (boolean, optional): Explicit confirmation required for org-wide sharing.`,
      inputSchema: {
        ...
        role: z
          .string()
          .describe("Application role to assign (e.g. employee, manager)."),
        user_email: z
          .string()
          .optional()
          .describe("Email of user to share with."),
        user_id: z.string().optional().describe("User UUID to share with."),
        group_name: z
          .string()
          .optional()
          .describe("Group name to share with. Use * for whole organization."),
        scope: z
          .enum(["user", "group", "org"])
          .optional()
          .describe(
            "Sharing scope. 'org' shares with all organization members.",
          ),
        expires_at: z
          .string()
          .optional()
          .describe("Optional ISO-8601 share expiration timestamp."),
        confirm: z
          .boolean()
          .optional()
          .describe("Must be true when sharing with the whole organization."),
      },
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
      ...
      try {
        ...
        if (
          ...
          throw new CliError({
            ...
            message:
              "Must specify either user_email, user_id, group_name, or scope='org'.",
        ...
        const targetDesc =
          args.user_email ||
          args.user_id ||
          (args.scope === "org" ? "Whole Organization" : args.group_name);
        const outputData = {
          action: "share_added",
          app_id: args.app_id,
          share_id: result.id,
  ...
  - confirm (boolean, optional): Explicit confirmation to revoke access.`,
      inputSchema: {
        ...
        confirm: z
          .boolean()
          .optional()
          .describe("Must be true to authorize share revocation."),
      },
      annotations: {
        readOnlyHint: false,
  ...
  - tail (number, optional): Number of recent log lines to retrieve (default: 50).`,
    ...
    async (args) => {
      try {
        ...
        const delimitedText = formatUntrustedText(
          sliced.join("\n"),
      ...
      try {
        ...
        const outputData = {
          ...
          versions: versions.map((v: any) => ({
            ...
            untrusted_description: formatUntrustedData(
              v.change_description || v.description,
...
Args:
  ...
  - reason (string, optional): Reason for the rollback.`,
      inputSchema: {
        ...
        target_version: z
          .number()
          .describe("Target version number to roll back to."),
        mode: z
          .enum(["code_only", "code_and_data"])
          .optional()
          .default("code_only")
          .describe("Rollback mode: 'code_only' or 'code_and_data'."),
        confirm: z
          .boolean()
          .optional()
          .describe("Required when mode is code_and_data."),
  ...
  - section (string, optional): Specific section to retrieve (e.g. 'manifest', 'sdk', 'database', 'connectors'). If omitted, returns the complete guide.`,
      inputSchema: {
        section: z
          ...
          .optional()
          .describe(
            "Optional section keyword to filter by (e.g. manifest, sdk, database, connectors).",
          ),
      },
      annotations: {
        readOnlyHint: true,
      ...
      try {
        ...
        if (args.section) {
          ...
          for (const line of lines) {
            if (
              line.startsWith("#") &&
              line.toLowerCase().includes(sectionQuery)
            ) {
              capturing = true;
              captured.push(line);
            } else if (
